...
function doRectanglesOverlap(rect1, rect2) {
  const nachX2 = rect2.left;
  const nachY2 = rect2.top;
  const width1 = rect1.left + rect1.width;
  const height1 = rect1.top + rect1.height;
  let fl = false;
  let fl1 = false;
  if (height1 >= nachY2) fl = true;
  if ((width1 >= nachX2)) fl1 = true;
  return ((fl) && (fl1));
}
...
function isInsideCircle(circle, point) {
  ...
  + (circle.center.y - point.y) ** 2) < circle.radius) return true;
  return false;
}
...
function isCreditCardNumber(ccn) {
  ...
  let sum = 0;
  if ((num.length - 1) % 2 !== 0) {
    for (let k = 0; k <= num.length - 1; k += 1) {
      let temp = 0;
      if (k % 2 === 0) temp = parseInt(num[k], 10) * 2;
      else temp = parseInt(num[k], 10);
      if (temp > 9) temp -= 9;
      sum += temp;
    }
  } else {
